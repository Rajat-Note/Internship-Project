...
export const EditFormModal = () => {
  ...
  const { type, isOpen, onClose, id, name, email, phone, hobbies } = useModal();
  const { editInfo } = useInfoStore();

  const isModalOpen = isOpen && type === "EditInfo";
  ...
  useEffect(() => {
    ...
    if (hobbies) {
      ...
      form.setValue("hobbies", allHobbies);
    }
  }, [form, name, email, phone, hobbies]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      editInfo(response.data._id, response.data);
